function [finalans] =  neweval_projkernel(D, input, output, N, trainsize, k, argsv)
...
minval = [];
for i = 1:1
	[mgp2, mgp2_nlml] = train_mat_precompute(mgp2, 10, [400 400], k)
	if isempty(op)
		minval = mgp2_nlml;
		op = mgp2.hyp;
	else
...
projkmat = reshape(mgp2.hyp(1:k*D), k, D);
