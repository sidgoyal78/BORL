%% train_lrr.m
%
%   function [gpmodel nlml] = train_lrr(gpmodel, dump, iter)
%
% *Input arguments:*
%
...
covfunc = {'covSum', {'covSElrr', 'covNoise'}};        % specify ARD covariance
...
if ~isfield(gpmodel,'hyp')  % if we don't pass in hyper-parameters, define them
  gpmodel.hyp = zeros(2,E); nlml = zeros(1,E);
  lh = repmat([0 -1]',1,E);                     % init hyp length scales
  lh(1,:) = log(std(gpmodel.targets)) ;                     %  signal std dev
  lh(2,:) = log(std(gpmodel.targets)/10) ;                  % noise std dev
else
